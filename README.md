...
uses the same bounded batches and combines the results newest-first.
...
stack in `Log`. Every current-state and history `Log` starts with the process's
operating system hostname in brackets; normal records contain only that prefix.
If the hostname is unavailable, the prefix is `[unknown]`. History insertion
and pruning are best effort. SQL errors from either operation do not change a
